...
public class ImportDate {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 2) 내장 클래스 (설계까지만 되어있음(1단계 생략), 안에 어떤기능이 있는지 공부해야만 알 수 있음)
		 	Scanner, System, Date
		 */
		...
		System.out.println(d1); // you remember!!!, toString()를 오버라이딩함
		
		System.out.println(d1.toLocaleString()); // 현재 시간 더깔끔하게 나옴
								//한줄그인이유 : 약간 오래된 명령어라는 뜻
	
		//Q) 출력은 오늘은 OO년 OO월 OO일로 나오게 짜기 	tip!! d1.까지만찍으면 해당 Date안에있는 메소드들이 보임 동그라미에 줄그인것들!! 
		System.out.println(d1.getYear()+"년"+d1.getMonth()+"월"+d1.getDate()+"일"+d1.getDay()+"요일입니다");
		//			window기준 1900년이후라서		c언어가 0부터시작			이상없음			0은 일요일~ 6은 토요일
		// 맞는결과 나오게 하려면
		String [] yoil = {"일", "월", "화", "수", "목", "금", "토"}; // getDay가 숫자로 되어있기 때문에
		String str1 = "오늘은 "; // str1이란 String에 문자를 다 더하는것
		...
		d1.setDate(13+99); //get으로 받았으니 set으로 값을 보내줄때 오늘날짜 +100일을 넣어주면됨
